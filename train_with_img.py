...
def load_model_from_config(model, ckpt, verbose=False):
    # 
    print(f"Loading model from {ckpt}")
    # 
    pretrained_checkpoint = torch.load(ckpt, map_location="cpu")
    if("model" in pretrained_checkpoint.keys()):
        sd = pretrained_checkpoint["model"]
    # For finetuning a pretrianed model from Facebook, ckpt does not have have "model"key
        m, u = model.load_state_dict(sd, strict=False)
    else:
        m, u = model.load_state_dict(pretrained_checkpoint, strict=False)
...
def main(args):
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(loader):
            ...
            frame_years = video_data["frame_metadata"]["frame_years"]
            frame_months = video_data["frame_metadata"]["frame_months"]
            
        
            frame_years = frame_years.to(device)
            frame_months = frame_months.to(device, dtype = torch.float32)
            frame_metadata = {
                "frame_years": frame_years,
                "frame_months": frame_months
            }
            ...
            if args.extras == 78: # text-to-video
                ...
                model_kwargs = dict(y = frame_metadata, use_image_num=args.use_image_num)
                # print("model conditions", model_kwargs)
            else:
                model_kwargs = dict(y=None, use_image_num=args.use_image_num)

            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            print(f"tensors shapes: x: {x.shape}, t: {t.shape}, t:{t.shape}")
