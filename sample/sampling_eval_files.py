...
try:
    ...
    from utils import find_model, get_sample_meta_data_template_dict
except:
...
def save_image_decoded(latents, ip_images, vae, args,ip_image_folder, start_year = 1997, iteration = 0):
    ...
    save_image(ip_op, save_path, nrow=6, normlize = True, value_range=(-1, 1))
    return save_path
...
def normalize_year(year, base_year = 2010, scale = 1000, max_year = 2050):
    # base_year = 2000
    # max_year = 2024
...
def main(args):
    ...
    all_eval_files = list(Path(args.eval_images_path).glob("**/"))
    
    for j, ip_images in enumerate(all_eval_files[1:]):
    ...
    # 
        ...
        start = 0
        num_batches = len(ip_images_path_list)//args.num_frames
        for k in range(num_batches):
            end_frame_index = len(ip_images_path_list) - ((len(ip_images_path_list) - start) - args.num_frames)if k == num_batches - 1 else start + args.num_frames
            image_tensors = [load_image_from_list(args, ip_images_path_list[i]).to(device, dtype = torch.float32) for i in range(start, end_frame_index) if ".json" not in ip_images_path_list[i].as_posix()]
            ...
            orig_years = [int(img_path.as_posix().split('/')[-1].split("_")[0]) for img_path in ip_images_path_list[start: end_frame_index]]

            year_tensors = convert_years_to_tensors(start_year=orig_years[0], num_frame=args.num_frames)
            month_tensors = encode_months_to_tensors(start_month = args.start_month, num_frames = args.num_frames)
            print("Year tensors", year_tensors)
            
            # orig_years = [int(img_path.as_posix().split('/')[-1].split("_")[0]) for img_path in ip_images_path_list[start:start+6]]
            ...
            save_each_frame(sampled_frames, ip_images_path_list, vae, args, ip_images_folder_id, orig_years, start)
            start = start + args.num_frames #if(len(ip_images_path_list) - start + args.num_frames)
            
            # difference = len(ip_images_path_list) - start+1
            # start = start if difference >= args.num_frames else start - (args.num_frames - difference)
            save_image_decoded(sampled_frames, image_tensors, vae, args,ip_images_folder_id, start_year=orig_years[0], iteration = j)

            video_save_path = convert_tensors_to_video(samples, vae,is_ddim_inv_noise = False, file_suffix=file_suffix, args = args, start_year=orig_years[0])
            sample_metadata_dict.update({
