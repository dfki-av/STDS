...
try:
    ...
    from utils import find_model, get_sample_meta_data_template_dict, create_overlayed_difference_map
except:
...
def convert_tensors_to_video(samples, vae, is_ddim_inv_noise = False, file_suffix = '', args = None, start_year = 1997):
    ...
    frames_with_text = []
    for i, frame in enumerate(samples[0]):  # Process each frame in the first video
        # Convert tensor frame to PIL Image for text overlay
        frame_image = ((frame * 0.5 + 0.5) * 255).clamp(0, 255).to(torch.uint8).cpu()
        frame_image = frame_image.permute(1, 2, 0).numpy()
        pil_frame = Image.fromarray(frame_image)
        year = start_year + i
        # Draw text on the frame
        draw = ImageDraw.Draw(pil_frame)
        text = f"{year}"  # Customize this text as needed
        font = ImageFont.load_default()
        text_position = (10, 10)  # Position of text in the frame
        text_color = (255, 165, 0)  # Text color (yellow)
        # "orange": (255, 165, 0),
        draw.text(text_position, text, font=font, fill=text_color)

        # Convert back to tensor and add to frames list
        frames_with_text.append(torch.from_numpy(np.array(pil_frame)).permute(2, 0, 1))
    # video_ = ((samples[0] * 0.5 + 0.5) * 255).add_(0.5).clamp_(0, 255).to(dtype=torch.uint8).cpu().permute(0, 2, 3, 1).contiguous()
    video_ = torch.stack(frames_with_text).permute(0, 2, 3, 1).contiguous()  # Shape: (f, h, w, c)
...
def save_each_frame(latents, ip_images_path_list, vae, args, ip_images_folder_id, years, start):
    ...
    file_names = [filename.replace(filename[:4], str(years + i)) for i, filename in enumerate(file_names)]
    save_folder = f"{save_folder}/{num_folders}-{str(years)}"
    ...
    for i in range(args.num_ip_images):
        ...
        # frame_image = ((op_frame + 1) * 127.5).clamp(0, 255).to(torch.uint8).cpu()
        ...
        # # "orange": (255, 165, 0),
        # draw.text(text_position, text, font=font, fill=text_color)
        # op_frame_w_text = torch.from_numpy(np.array(pil_frame).astype(np.float32) / 255.0).permute(2, 0, 1).to(device)
        # op_frame = rearrange(op_frame, '(f c) h w -> f c h w', f = 1)
        # ip_op_tensors.append(op_frame_w_text)
...
def save_image_decoded(latents, ip_images, vae, args,ip_image_folder, start_year = 1997, iteration = 0):
    # 
    ip_op_tensors = []
    # 
    
    for i in range(args.num_ip_images):
        ip_op_tensors.append(clamp_img(ip_images[i]))
        # sampled_frames = [latents[i][:,k,:,:,:] for k in range(args.num_frames)]
        op_frame = vae.decode(latents[i]/ 0.18215).sample
        # frame_image = ((op_frame * 0.5 + 0.5) * 255).clamp(0, 255).to(torch.uint8).cpu()
        frame_image = ((op_frame + 1) * 127.5).clamp(0, 255).to(torch.uint8).cpu()
        ...
        gen_img_np = op_frame[0].permute(1,2,0).cpu().numpy()
        # gray scale
        ...
        draw.text(text_position, text, font=font, fill=text_color)
        op_frame_w_text = torch.from_numpy(np.array(pil_frame).astype(np.float32) / 255.0).permute(2, 0, 1).to(device)
        op_frame_w_text = rearrange(op_frame_w_text, '(f c) h w -> f c h w', f = 1)
        ip_op_tensors.append(op_frame_w_text)
    ...
    ip_op = torch.cat(ip_op_tensors).contiguous()

    # ip_op = torch.cat([ip_images, decoded_latents], dim = 0)
    num_images = len(list(Path(args.decoded_latents_save_path).glob("**/*.*")))
    num_folders = len(list(Path(args.decoded_latents_save_path).glob("**/")))
    print("num images", num_images)
    latents_save_path = Path(args.decoded_latents_save_path).joinpath(ip_image_folder+"_"+ str(iteration))
    os.makedirs(latents_save_path, exist_ok= True )
    save_file_name = "ip_1997_op_" + str(start_year) + "_" + args.file_suffix + "_"+ str(num_images)+ '_.png'
    save_path = Path.joinpath(Path(latents_save_path), save_file_name)
    # ip_op = clamp_img(ip_op)
    print("IP Images:", save_path)
    save_image(ip_op, save_path, nrow=8, normlize = True, value_range=(-1, 1))
    return save_path
...
def perform_ddim_inversion(diffusion, model, init_image, y, using_cfg, args):
    ...
    #     model_kwargs = dict(y=y, cfg_scale=args.cfg_scale, use_fp16=False)
...
def normalize_year(year, base_year = 2015, scale = 1000, max_year = 2100):
    # base_year = 2000
    # upper_year = 2024
    # mid_year = (base_year + max_year) / 2    
...
def extract_metadata(metadata_dict, args):
    ...
    for frame_no in metadata_dict["year_wise_metadata"]:
        ...
        # year_norm, lat_norm, lon_norm, ndvi_norm = metadata_normalize(numeric_metadata)
        year_norm = metadata_normalize(numeric_metadata)
        frame_metadata["frame_years"].append(year_norm)
        # frame_metadata["ndvi"].append(ndvi_norm)
        # frame_metadata["latitude"].append(lat_norm)
        # frame_metadata["longitude"].append(lon_norm)
...
# print("Metadata before normalize", metadata)
...
def get_histogram_segmentaed_images(image_path):
    ...
    # save_path = "/netscratch/kashyap/Latte/run_files/deforestation_256_with_guidance_segmentation_images/segmentation_images"
...
def calculate_structural_diffs(ip_images_path_list, args):
    ...
    for i in range(num_comparisons):
        ...
        ssim_value, _ = ssim(gray1, gray2, full=True, data_range=gray1.max() - gray1.min())
        structural_diff = 1- ssim_value
        scale = 1000
...
def main(args):
    ...
    for j, ip_images in enumerate(args.ip_image_folder_list):
        ip_images_path_list = list(Path(ip_images).glob("**/*.*"))   
        ip_images_path_list = sorted(ip_images_path_list, key=lambda item: int(item.name.split('.')[0].split('_')[0]) if item.name.split(".")[1] == "png" else 0)
        ip_images_folder_id = Path(ip_images).name
    ...
    # 
        ...
        # if(len(ip_images_path_list) and ip_images_path_list[0].as_posix().split(".")[1] == "json"):
        #     metadata = ip_images_path_list.pop(0)
        # metadata_json_path = list(filter(lambda x: ".json" in x.as_posix() , ip_images_path_list))
        # with open(metadata, "r") as f:
        #     metadata = json.load(f)
        # metadata = extract_metadata(metadata, args)
        # print("ip_images_path_list", ip_images_path_list, len(ip_images_path_list))
        

        num_ip_images_to_use = args.num_ip_images
        source_image_folder = "/netscratch/kashyap/Temporal_diffusion/GEE_data/processed_data/Landsat_from_1984/deforestation/hq_data/downsampled_256x256/red"
        start = 0
        image_tensors = [load_image_from_list(args, ip_images_path_list[i]).to(device, dtype = torch.float32) for i in range(start, start+num_ip_images_to_use) if ".json" not in ip_images_path_list[i].as_posix()]
        if(args.use_structural_diff):
            structure_diff_list = calculate_structural_diffs(ip_images_path_list=ip_images_path_list, args=args)
        ...
        # s = []
        for init_year in args.init_years_list:
            z = torch.cat(init_images_latents).to(device)
            z = z.unsqueeze(0) # introduce batch dimension
            
            year_tensors = convert_years_to_tensors(start_year=init_year, num_frame=args.num_frames)
            month_tensors = encode_months_to_tensors(start_month = args.start_month, num_frames = args.num_frames)
            print("Year tensors", year_tensors)
            
            orig_years = [int(img_path.as_posix().split('/')[-1].split("_")[0]) for img_path in ip_images_path_list[start:start+args.num_frames]]
            print("Orig Years:", orig_years)
            orig_year_tensors = convert_orig_years_to_tensors(orig_years[:args.num_frames])
            ...
            if(num_ip_images_to_use < args.num_frames):
                ...
                # random_tensors = torch.randn(z.shape[0], args.num_frames - num_ip_images_to_use, z.shape[2], z.shape[3], z.shape[4], device = device) # introduce  random tensors apart from the unsqueezed dimension 
            ...
            if(args.extras == 3):
                ...
                # frame_metadata = {"frame_years": orig_year_tensors.unsqueeze(0), "frame_months": month_tensors.unsqueeze(0)}
                ...
                if(args.use_guidance_images):
                    frame_metadata = {"frame_years": year_tensors.unsqueeze(0), "frame_months": month_tensors.unsqueeze(0), "images": torch.stack(guidance_image_tensors, dim = 1)}
                            ...
                            # for i, g in enumerate(frame_metadata["images"]):
                                # g = guidance_image
            ...
            if args.sample_method == 'ddim':
                ...
                for sample_progress in ddim_sample_progression:
                    ...
                    # second_frame = sample_progress["sample"][:,1,:,:,:]
                    # noisy_frame = sample_progress["sample"][:,-1,:,:,:]
            ...
            save_each_frame(sampled_frames, ip_images_path_list, vae, args, ip_images_folder_id, init_year, start)
            save_image_decoded(sampled_frames, image_tensors, vae, args,ip_images_folder_id, start_year=init_year, iteration = j)

            video_save_path = convert_tensors_to_video(samples, vae,is_ddim_inv_noise = False, file_suffix=file_suffix, args = args, start_year=init_year)
            sample_metadata_dict.update({
            ...
            # video_save_path = os.path.join(args.save_video_path, f'sample_{num_videos}.mp4')
            # print(video_save_path)
            # imageio.mimwrite(video_save_path, video_, fps=1, quality=9)
            # print('save path {}'.format(args.save_video_path))
