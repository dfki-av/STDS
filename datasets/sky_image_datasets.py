...
class SkyImages(data.Dataset):
    ...
    def __getitem__(self, index):
        # 
        # print("get data in  dataset", index)
        ...
        for i in range(self.use_image_num):
            while True:
                try:      
                    ...
                    image = torch.as_tensor(np.array(Image.open(video_frame_path).convert('RGB'), dtype=np.uint8, copy=True)).unsqueeze(0)
                    images.append(image)
                    break
    ...
    def load_video_frames(self, dataroot):
        ...
        for _, meta in enumerate(frame_list):
            root = meta[0]
            try:
                frames = sorted(meta[2], key=lambda item: int(item.split('.')[0].split('_')[0]))
            except:
                print(meta[0]) # root
                print(meta[2]) # files
