...
from transformers import CLIPTokenizer, CLIPTextModel, CLIPImageProcessor, CLIPModel, CLIPProcessor
import torch
import transformers
...
class CLIPImageEmbedder(nn.Module):
    ...
    def forward(self, image_tensor):
        ...
        # inputs = self.processor.feature_extractor(images = image_tensor, return_tensor = "pt", padding = True)
        normalized_inputs = self.processor.feature_extractor(
                            images=image_tensor,
                            return_tensors="pt",           
    ...
    # with torch.no_grad():
        ...
        normalized_inputs = normalized_inputs.to("cuda")
        self.model = self.model.to("cuda")
        self.model = self.model.eval()
        # self.model = self.model.
        image_embeddings = self.model.get_image_features(pixel_values=normalized_inputs)
