...
from models.clip import TextEmbedder, CLIPImageEmbedder
...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{experiment_index:03d}-{model_string_name}-{num_frame_string}-{args.dataset}-{args.model_special_name}"  # Create an experiment folder
    ...
    if args.resume_from_checkpoint:
        ...
        experiment_dir = args.resume_from_checkpoint
        # 
        print("experiment dir", experiment_dir)
        dirs = os.listdir(os.path.join(experiment_dir, 'checkpoints'))
        print("dirs", dirs)
        ckpts = [d for d in dirs if d.endswith("pt")]
        ckpts = sorted(dirs, key=lambda x: int(x.split(".")[0]))

        path = ckpts[-1]
        logger.info(f"Resuming from checkpoint {path}")
        # model.load_state_dict(os.path.join(experiment_dir,'checkpoints', path))
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, video_data in enumerate(loader):
            ...
            x = video_data['video'].to(device, non_blocking=True, dtype = torch.float32)
            ...
            elif args.extras == 3: # conditioning on metadata (numeric data)
                for key, value in video_data["frame_metadata"].items():
                    if(type(value) == list):
                        for k in range(len(value)):
                            video_data["frame_metadata"][key][k] = video_data["frame_metadata"][key][k].to(device, dtype = torch.float32)
                    else:
                        video_data["frame_metadata"][key] = video_data["frame_metadata"][key].to(device, dtype = torch.float32)
                
                frame_metadata = video_data["frame_metadata"]
                if(args.use_guidance_images):
                    if(args.use_CLIP_image_embeddings):
                        
                        image_embedder = CLIPImageEmbedder()
                        guidance_image = frame_metadata["images"].squeeze(1).permute(0,3,1,2)
                        image_embeddings = image_embedder(guidance_image)

                    else:
                        frame_metadata["images"] = torch.stack(frame_metadata["images"], dim = 1).squeeze(2)
                        g = frame_metadata["images"]
                        g = rearrange(g, 'b f c h w -> (b f) c h w').contiguous()
                        g = vae.encode(g).latent_dist.sample().mul_(0.18215)
                        g = rearrange(g, '(b f) c h w -> b f c h w', b=b).contiguous()
                        frame_metadata["images"]= g
                model_kwargs = dict(y = frame_metadata)
            elif args.extras == 4 or args.extras == 5: # deforestation
                for key, value in video_data["frame_metadata"].items():
                    if(type(value) == list):
                        for k in range(len(value)):
                            video_data["frame_metadata"][key][k] = video_data["frame_metadata"][key][k].to(device, dtype = torch.float32)
